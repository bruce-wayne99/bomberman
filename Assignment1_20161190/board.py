...
class Board(Walls, Bricks):
    ...
    def CreateEmptyBoard(self):                     # Returns a empty board
            ...
            if i % 2 == 0:
                for j in range(4, self.columns - 4):
                    if j % 4 == 0 and Grid[i][j] != 'X':
                        ...
                        if (random() > 0.7 and
                            count < self.GetBrickslimit() and
                            (i != 2 or j != 4) and (i != 2 or j != 8) and
                                (i != 4 or j != 4)):
                            brick = Bricks(4, 2)
                            temp = brick.Brick
                            Grid = self.PopulateBoardWithElement(i,
                                                                 j, Grid, temp)
                            count = count + 1
        return Grid
            ...
            if i % 2 == 0:
                for j in range(4, self.columns - 4):
                    if j % 4 == 0 and grid[i][j] == ' ':
                        ...
                        ''' Random value is set to 0.9 so that enemies would be
                            distributed more evenly
                        '''
                        if (random() > 0.9 and
                            count < max_enemies and
                            (i != 2 or j != 4) and
                                (i != 2 or j != 8) and (i != 4 or j != 4)):
                            enemy = Enemy(i, j, 2)
    ...
    # The board holds the list of enemies
    ...
    def RemoveEnemyFromList(self, x, y):
        for i in range(len(self.enemies)):
            if (self.enemies[i].get_lefttop_x() == x and
                    self.enemies[i].get_lefttop_y() == y):
